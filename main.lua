...
if (_G.AutofarmSettings.Credits ~= "Fentanyl Farm - King") then 
...
local Log = function(Msg)
	print(os.date("%X").." | Fentanyl Farm - "..Msg)
end
...
task.spawn(function()
	while true and task.wait(_G.AutofarmSettings.LogInterval * 60) do
        local s, e = pcall(function()
            SendLog(_G.AutofarmSettings.Webhook, {Player.Name, Player.UserId, WALLET.Text, PROFIT.Text, TIMER.Text, BROKEN.Text, "| Fentanyl Farm - KING"})
        end)
        if (e) then 
